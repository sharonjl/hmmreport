...
Technology from our hand-watches to parts for space stations are built by automated machines. Today, it is an important part of our lives. And in the future it will be even more important as we start interacting with intelligent agents to accomplish our tasks. The key difference between today's robots and tomorrows robot is not the computing power or sensing capabilities it will be their seamless interaction with their users. As we know it a robot that manufactures parts for automobiles certainly does not need to be socially friendly with its human operaters. These robots do what they are told, without question; but that is the purpose behind them, build parts continuously. But we want to look into robots that might one day service users, help you make coffee, assist someone in a grocery store, help children cross traffic lights, be a friendly companion, act as a guide dog, or help elders in need around the house. These robots are not performing tedious tasks on a daily basis, but are to be interactive with humans, and socialize with them. For this to happen, robots need to sense and interpret human interaction, their gestures and body language.
...
\section{Data set}
As in any machine learning algorithm, a data set plays an important role in enabling the algorithm to train, classify and re-train. There are two basic ways throug which data can be captured for gesture recognition, data captured through a video camera, and data captured through a 3D marker tracking system. \citet{Morie200731} provide a summary of their experiments in a creating markerless visual tracking system inspired by the brain; for face and object recognition. The camera captures a sequence of images that is used to extract features later in the process. The sophistication, or the details, of the data captured is reliant on the camera itself. A hi-definition camera would capture more details versus a low definition camera, likewise a camera that is able to collect color data can provide additional details of the scene.
...
The order of the symbols in the sequence of observations matter. Changing the order of the symbols in the sequence would imply a very different gesture or emotion. As such, each observation sequence is said to occur at specific arbitrarly defined states in a gesture. A gesture can have many states depending on the complexity of the gesture. We assume the gestures to have three observable states: 
\begin{enumerate}
	\item A start state
	\item A middle state
	\item and an end state
\end{enumerate}
Between these states there are many hidden states, or transition states, that we are not concerned with. If we consider someone giving a wave, there are three visible states we notice. The start state where the hands are relaxed, the middle state where the hands are bent in front of the person, and the final state where the person has their palm open and tilted about the wrist. However, the number of states and the visibility of these states is not guaranteed. One can convey a hand wave with two postures, a posture with the wrist tilted to the left and a posture with the wrist titled to the right. With this interpretation one cannot say that a gesture must have a start state, a middle or an end state that are visible to a human observer. A gesture is considered a temporal pattern, as such the states are considered hidden and non-observable. However, one can observe the output from a hidden state. And a HMM can determine some information about the states through the sequence of observations. Hence, the application of Hidden Markov Models for gesture recognition. The number of states are optimized through experimentation of various gestures gestures. Different gestures have different states, and certain states improve the accuracy of using HMMs to classify similar gestures. 
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.0\textwidth]{codebookoverhead.pdf}
	\caption{Graph of codebook creation time versus number of features in a feature vector}
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.0\textwidth]{codebookoverhead2.pdf}
	\caption{Graph of codebook creation time versus number of feature vectors to cluster}
	\label{fig:codebookoverhead2}
\end{figure}


Table \ref{tab:timedperformance} also lists the overhead incurred for HMM training. This is the total training time for all four emotions. Individually, the training of each emotion depends on the observation sequence pattern and the rate of convergence of the HMM probability matrices. 
